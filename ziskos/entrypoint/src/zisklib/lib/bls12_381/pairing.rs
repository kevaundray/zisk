...
/// - false otherwise
#[no_mangle]
pub unsafe extern "C" fn bls12_381_pairing_check_c(pairs: *const u8, num_pairs: usize) -> bool {
    ...
    for i in 0..num_pairs {
        ...
        if !is_on_curve_bls12_381(&g1_u64) {
            return false;
        }

        // Verify G1 point is in subgroup
        if !is_on_subgroup_bls12_381(&g1_u64) {
            return false;
        }
        ...
        if !is_on_curve_twist_bls12_381(&g2_u64) {
            return false;
        }

        // Verify G2 point is in subgroup
        if !is_on_subgroup_twist_bls12_381(&g2_u64) {
            return false;
        }
