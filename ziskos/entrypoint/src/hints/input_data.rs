...
use zisk_common::HINT_INPUT_DATA;
...
pub unsafe extern "C" fn hint_input_data(input_data_ptr: *const u8, input_data_len: usize) {
    ...
    let mut w = crate::hints::HINT_BUFFER.begin_hint(HINT_INPUT_DATA, 8 + input_data_len + pad, false);
    ...
    w.write_hint_data_slice(&input_data_len_bytes);
    w.write_hint_data_ptr(input_data_ptr, input_data_len);
    if pad > 0 {
...
fn input_data_register_meta() {
    crate::hints::metrics::register_hint(HINT_INPUT_DATA, stringify!(input_data).to_string());
}
