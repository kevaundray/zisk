...
- Ziskemu execution:
```bash
ziskemu -i build/input.bin -x -e target/riscv64imac-unknown-none-elf/release/hello_world
```
...
```bash
ziskemu -i build/input.bin -x -e target/riscv64imac-unknown-none-elf/debug/hello_world
```
...
```bash
mpirun --bind-to none -np <number_processes> -x OMP_NUM_THREADS=<number_of_threads_per_process> target/release/cargo-zisk prove -e target/riscv64imac-unknown-none-elf/release/sha_hasher -i build/input.bin -w $HOME/.zisk/bin/libzisk_witness.so -k $HOME/.zisk/provingKey -o proof -a -y
```
### Verify the Proof
```bash
